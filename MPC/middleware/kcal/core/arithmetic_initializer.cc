...
ArithmeticInitializer::ArithmeticInitializer() : opts_(DG_InitArithmeticOpts()) {}

int ArithmeticInitializer::InitializeTeeCtx(void *teeConfig, int worldSize, DG_TeeCtx **teeCtx)
{
    ...
    rv = opts_.negotiateSeeds(*teeCtx);
    if (rv != DG_SUCCESS) {
        return rv;
    }
    return DG_SUCCESS;
...
void ArithmeticInitializer::ReleaseTeeCtx(DG_TeeCtx **teeCtx)
{
    if (teeCtx && *teeCtx) {
