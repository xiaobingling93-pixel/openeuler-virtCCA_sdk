...
1. 需获取 `kcal` 包，含 `include` 和 `lib` 目录
...
## clone 蚂蚁 psi 仓，应用 patch

注：目前已适配蚂蚁 `tag: psi-v0.6.0.dev250507` 版本的 `psi`库
...
编译完成后，在`bazel-bin/psi/apps/psi_launcher`目录下生成`main`可执行文件，后续可通过执行`./bazel-bin/psi/apps/psi_launcher/main --config xxx.json`进行验证

## 将二进制部署至 virtCCA 内

可让`/home/admin/dev/psi`的目录结构与`virtCCA`内部保持一致，方便进行测试

# kcal psi 在蚂蚁 psi 库上的测试

编译完成后，进入`/home/admin/dev/psi`目录，步骤可参考：`examples/psi/README.md`​
