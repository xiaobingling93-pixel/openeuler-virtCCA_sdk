...
def test_basic_arithmetic(context, is_server: bool):
    ...
    if is_server:
        print("\nServer: Processing arithmetic operations...")
        ...
        print("\nClient: Processing arithmetic operations...")
...
def test_comparison_operations(context, is_server: bool):
    ...
    less_op.run([share1, share2], less_out_share)  # 10<15, 20<20, 30<25, 40<50 = [1,0,0,1]
    less_equal_out_share = kcal.MpcShare()
    less_equal_op.run([share1, share2], less_equal_out_share)  # 10<=15, 20<=20, 30<=25, 40<=50 = [1,1,0,1]
    greater_out_share = kcal.MpcShare()
    greater_op.run([share1, share2], greater_out_share)  # 10>15, 20>20, 30>25, 40>50 = [0,0,1,0]
    greater_equal_out_share = kcal.MpcShare()
    greater_equal_op.run([share1, share2], greater_equal_out_share)  # 10>=15, 20>=20, 30>=25, 40>=50 = [0,1,1,0]
    equal_out_share = kcal.MpcShare()
    equal_op.run([share1, share2], equal_out_share)  # 10=15, 20=20, 30=25, 40=50 = [0,1,0,0]
    no_equal_out_share = kcal.MpcShare()
    no_equal_op.run([share1, share2], no_equal_out_share)  # 10!=15, 20!=20, 30!=25, 40!=50 = [1,0,1,1]

    # Reveal results
    less_output = []
    less_equal_output = []
    greater_output = []
    greater_equal_output = []
    equal_output = []
    no_equal_output = []

    reveal_share_op.run(less_out_share, less_output)
    reveal_share_op.run(less_equal_out_share, less_equal_output)
    reveal_share_op.run(greater_out_share, greater_output)
    reveal_share_op.run(greater_equal_out_share, greater_equal_output)
    reveal_share_op.run(equal_out_share, equal_output)
    reveal_share_op.run(no_equal_out_share, no_equal_output)

    print(f"LESS (input1<input2) result: {less_output}")
    print(f"LESS_EQUAL (input1<=input2) result: {less_equal_output}")
    print(f"GREATER (input1>input2) result: {greater_output}")
    print(f"GREATER_EQUAL (input1>=input2) result: {greater_equal_output}")
    print(f"EQUAL (input1==input2) result: {equal_output}")
    print(f"NO_EQUAL (input1!=input2) result: {no_equal_output}")
...
def test_aggregate_operations(context, is_server: bool):
    ...
    sum_op.run([share], sum_out_share)  # [10+5, 20+15, 30+25, 40+35] = [15, 35, 55, 75]
    avg_op.run([share], avg_out_share)  # Average of each position
    max_op.run([share], max_out_share)  # Max of each position: [10,20,30,40]
    min_op.run([share], min_out_share)  # Min of each position: [5,15,25,35]
...
def test_share_management(context, is_server: bool):
    ...
    # Test data with different types, default server holds data
    test_inputs = [
...
def create_parser():
    ...
    try:
        ...
        group.add_argument(
            "--server", action="store_true", default=False, help="start server"
        )
        group.add_argument(
            "--client", action="store_true", default=False, help="start client"
        )
