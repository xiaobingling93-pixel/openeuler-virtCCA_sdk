...
2. 配置远程证明组件启动依赖的配置文件。
   1.  进到目标目录。
       ...
       `cd /home/coco/remote\_attestation`
...
3. 启动kbs、rvps和as常驻服务。
   1.  启动kbs。
       ...
       RUST\_LOG=DEBUG ./kbs -c ./kbs-config-grpc.toml

       ![](figures/zh-cn_image_0000002338371593.png)

   2.  启动rvps。

       RUST\_LOG=DEBUG ./rvps -a 127.0.0.1:50003 -c ./rvps-config.json

       ![](figures/zh-cn_image_0000002304452338.png)

   3.  启动as。

       RUST\_LOG=DEBUG ./grpc-as -s 127.0.0.1:50004 -c ./as-config.json
