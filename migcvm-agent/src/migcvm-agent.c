...
static void custom_handler(const struct socket_msg *msg, int conn_fd, mig_agent_args *args)
{
    RTLS_INFO("Custom handling: cmd=%s, payload=%s\n",
           msg->cmd, msg->payload);

    if (strcmp(msg->cmd, "s") == 0) {
        payload_decode(msg, args->srv_ip);
...
void *rats_tls_server(void *arg)
{
    mig_agent_args *args = (mig_agent_args *)arg;
    if (args->agent_type < 0) {
        return -1;
    } else if (args->agent_type == MIG_TYPE_SERVER) {
        return rats_tls_server_startup(args);
    } else {
...
int main(int argc, char **argv)
{
    ...
    int ret = socket_agent_start_with_handler(&cfg, custom_handler); /* host socket */
    if (ret != 0) {
